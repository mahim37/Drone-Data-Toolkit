...
def crc64(seed, buffer, buffer_size):
    crc = seed

    for i in range(buffer_size):
        tab_index = (buffer[i] ^ crc) & 0xFF
        crc = crc_table[tab_index] ^ (crc >> 8)

    return crc


def get_scramble_bytes(record_type, key_byte):
    # Set up the (8-byte) buffer to compute the CRC over
    data_for_buffer = 0x123456789ABCDEF0 * key_byte
    buffer_to_crc = bytearray()

    # Fill in the buffer in little-endian order
    for _ in range(8):
        buffer_to_crc.append(data_for_buffer & 0xFF)
        data_for_buffer >>= 8

    # Compute a CRC over this buffer
    crc_result = crc64(record_type + key_byte, buffer_to_crc, len(buffer_to_crc))

    # Unpack the CRC result into result_scramble_bytes in little-endian order
    result_scramble_bytes = bytearray()
    for _ in range(8):
        result_scramble_bytes.append(crc_result & 0xFF)
        crc_result >>= 8

    return result_scramble_bytes

# def crc64_remainder(data, polynomial):
#     remainder = 0
#     data = data << 56  # Shift data left to align with CRC polynomial
#     for _ in range(64):
#         if (remainder ^ data) & 0x8000000000000000:
...
#
#
# def find_crc64_polynomial(crc_table):
#     polynomial =
#     # print(polynomial)
#     for data, crc in enumerate(crc_table):
#         if crc64_remainder(data, polynomial) != crc:
#             valid = False
#             break
#     if valid:
#         return polynomial
...
#
#
# polynomial = find_crc64_polynomial(crc_table)
# if polynomial is not None:
#     print(f"Reverse engineered polynomial: {hex(polynomial)}")
# else:
